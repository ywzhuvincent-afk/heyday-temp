...
describe("UI helpers", () => {
  ...
  it("filters manager alerts by alert type", () => {
    const alerts = [
      ...
      { id: "alert-2", type: "job_completed" },
      { id: "alert-3", type: "need_more_info" },
    ];

    assert.deepEqual(filterAlertsByType(alerts, "all").map((alert) => alert.id), ["alert-1", "alert-2", "alert-3"]);
    assert.deepEqual(filterAlertsByType(alerts, "job_completed").map((alert) => alert.id), ["alert-2"]);
  });
  ...
  it("filters employee jobs by dashboard status", () => {
    const jobs = [
      ...
      { id: "job-4", status: "revision_requested" },
      { id: "job-5", status: "job_completed" },
      { id: "job-6", status: "reviewed_billed" },
    ];
    ...
    assert.deepEqual(filterJobsByEmployeeStatus(jobs, "in_progress").map((job) => job.id), ["job-2", "job-3"]);
    assert.deepEqual(filterJobsByEmployeeStatus(jobs, "revision_requested").map((job) => job.id), ["job-4"]);
    assert.deepEqual(filterJobsByEmployeeStatus(jobs, "waiting_manager_review").map((job) => job.id), ["job-5"]);
    assert.deepEqual(filterJobsByEmployeeStatus(jobs, "closed_or_sent").map((job) => job.id), ["job-6"]);
  });
